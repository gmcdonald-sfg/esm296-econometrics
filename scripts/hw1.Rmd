...
We first find the t-statistic, which is calculated by dividing the slope by the heteroskedasticity-robust standard error (`r signif(0.73/0.1, 3)`). We next combine this with the degrees of freedom (`r 110 -1`) to determine a p-value of `r pt(0.73 / .1 , df = 110 - 1, lower=FALSE) %>% signif(3) %>% prettyNum()`. The slope is therefore statistically significant.
...
The null hypothesis is H0: β0 = 0. We first find the t-statistic, which is calculated by dividing the intercept by the heteroskedasticity-robust standard error (`r signif(19.6/7.2, 3)`). We next combine this with the degrees of freedom (`r 110 -1`) to determine a p-value of `r pt(19.6/7.2 , df = 110 - 1, lower=FALSE) %>% signif(3) %>% prettyNum()`. The slope is therefore statistically significant at the 1% level and we reject the null hypothesis.
...
The null hypothesis is H0: β1 = 0. We first find the t-statistic, which is calculated by dividing the slope by the heteroskedasticity-robust standard error (`r signif(0.73/0.1, 3)`). We next combine this with the degrees of freedom (`r 110 -1`) to determine a p-value of `r pt(0.73 / .1 , df = 110 - 1, lower=FALSE) %>% signif(3) %>% prettyNum()`. The slope is therefore statistically significant at the 5% level and we reject the null hypothesis.
